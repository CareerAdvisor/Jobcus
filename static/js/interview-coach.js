...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadCoachPreview(htmlOrUrl) {
    ...
    if (htmlOrUrl?.startsWith("<")) { coachFrame.srcdoc = htmlOrUrl; }
    else { coachFrame.src = htmlOrUrl || "about:blank"; }
  ...
  let currentQuestion = "";
  let previousRole = "", targetRole = "", experience = "";
  let asking = false;

  function setBusy(el, busy) { if (el) { el.disabled = !!busy; el.setAttribute("aria-busy", busy ? "true" : "false"); } }
  function setLive(el, html) { if (el) el.innerHTML = html; }
  function ensureVisible(el) { try { el?.scrollIntoView({ behavior: "smooth", block: "start" }); } catch {} }
  ...
  async function getNextQuestion() {
    ...
    try {
      ...
      currentQuestion = data?.question || "Please sign up or log in to use this feature.";
  ...
  if (roleForm) {
    roleForm.addEventListener("submit", async (e) => {
      ...
      if (!prev || !targ || !exp) { alert("Please fill in your previous role, target role, and experience level."); return; }
      previousRole = prev; targetRole = targ; experience = exp;
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        const data = await apiPost("/api/interview/feedback", {
          question: currentQuestion, answer, previousRole, targetRole, experience,
        });
        ...
        if (tips.length) {
          ...
          setLive(suggestionsBox, `<h4>💡 Suggested Tips:</h4><ul>${tips.map(t => `<li>${escapeHtml(t)}</li>`).join("")}</ul>`);
        }

        history.push({ question: currentQuestion, answer, feedback: data?.feedback || "", tips });
        renderHistory();
        suggestionsBox.style.display = "block";
        feedbackBox.style.display = "block";
      } catch (err) {
  ...
  function renderHistory() {
    ...
    if (!history.length) { historyContent.innerHTML = "<p>No history yet.</p>"; return; }
